...
let list_user_mes = []
/*Hien thi danh sach nhung nguoi nhan tin vs user*/
function mes1() {
    text = ''
...
/*Ham ho tro tim kiem*/
$('.search_user_mes').keyup(function () {
    mes2($('.search_user_mes').val())
})
function mes2(s) {
    text = ''
    for (let i = list_mes.length - 1; i >= 0; i--) {
        ...
        if (user.username == list_mes[i][1]) {
            for (let j = 0; j < list_user.length; j++) {
                if (list_mes[i][0] == list_user[j].username) {
                    user3 = list_user[j]
                    break
                }
            }
            if (user3.fullname.indexOf(s) > -1 || user3.username.indexOf(s) > -1) {
                user2 = list_mes[i][0]
                check = true
            }
        }
        if (user.username == list_mes[i][0]) {
            for (let j = 0; j < list_user.length; j++) {
                if (list_mes[i][1] == list_user[j].username) {
                    user3 = list_user[j];
                    break;
                }
            }
            if (user3.username.indexOf(s) > -1 || user3.fullname.indexOf(s) > -1) {
                user2 = list_mes[i][1]
                check = true
            }
        }
...
$('.Nhap_tin_nhan').keypress(function (ev) {
...
$('.delete_mes').click(function () {
    let user3 = this.parentElement.querySelector('div p').textContent
    for (let i = 0; i < list_mes.length; i++) {
        if (list_mes[i][0] == user.username && list_mes[i][1] == user3 || list_mes[i][1] == user.username && list_mes[i][0] == user3) {
            list_mes = list_mes.filter((item) => item !== list_mes[i]);
            k = 1
            h = 1
            first_user_mes1 = undefined
            list_user_mes = []
            localStorage.setItem('mess',JSON.stringify(list_mes));
            mes1()
            Show_mes(h, first_user_mes1)
