...
/* Đẩy thông tin của user vào các thẻ, lấy các thông tin cần thiết từ localStore*/
...
for (let i = 0; i < list_user.length; i++) {
  for (let j = 0; j < list_user[i].followed.length; j++) {
    if (list_user[i].followed[j] == user.username) follower = follower + 1;
  }
}
...
$(".btnchagename").click(function () {
  ...
  if (
    /^[a-zA-Z\săâưôơéèẻẹếềểệễýỵỳỷùúụủũừứựửíìịỉóòọỏáàạảấầẩậắằằặđ]{2,}$/i.test(
      fullname
    )
  ) {
    for (let i = 0; i < list_user.length; i++) {
...
$(".btnchangeavata").click(function () {
  if (img_url == undefined) alert("Bạn chưa chọn ảnh");
  else {
    for (let i = 0; i < list_user.length; i++) {
      if (list_user[i].username == user.username) {
...
/*Lấy link ảnh khi đổi ảnh đại diện*/
...
/* Hiển thị tất cả các bài viết đã like*/
$(".Likes").click(function () {
  show(2);
});

/*Hiện thị các bài viết của người dùng*/
show(1);
function show(x) {
  console.log(x);
  ...
  if (list_tweet != undefined) {
    ...
    for (let i = list_tweet.length - 1; i > 0; i--) {
      ...
      for (let j = 0; j < user.followed.length; j++) {
        if (
          (x == 1 && list_tweet[i].user.username == user.username) ||
          (x == 2 && list_tweet[i].like.indexOf(user.username) >= 0)
        ) {
          check_user = true;
          console.log("ok");
          break;
        ...
        text =
          ...
          `<div class="container-fluid">
            ...
            <div class="col-1" style="padding-left:0 ;">
                <a href="#">
                    <img class="anh_avata" src="${
                      list_tweet[i].user.img
                    }" alt="">
                </a>
            </div>
      ...
      for (let i = 1; i < list_tweet.length; i++) {
        if (list_tweet[i].id == id) {
          ...
          this.parentElement.parentElement.querySelector(
            ".comment"
          ).textContent = list_tweet[i].comment.length;
          break;
        }
      }
    });
      ...
      if (
        ...
        for (let i = 1; i < list_tweet.length; i++) {
          if (list_tweet[i].id == this.id) {
            this.parentElement.querySelector(`#i${this.id}`).textContent =
              "like";
